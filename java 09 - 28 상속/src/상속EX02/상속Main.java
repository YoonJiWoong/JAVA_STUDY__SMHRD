	...
	public static void main(String[] args) {
		...
		while (true) {
			...
			if (choice == 1) {
				...
				// 3. 삼각형의 넓이 계산 후 출력
				System.out.println(Triangle.getName()+"의 넓이 :" + t.calArea()); //Triangle 라고 쓰면 안되!! 변수명 쓰기 Static 오류나옴
	//			System.out.print("삼각형의 넓이는"+new Triangle(base, height).calArea()); 이런식으로 바로 써도 된다!!

				// 똑 같은 메소드를 상속받았지만 재정의 해서 다르게 사용!! -> 다형성!
			} else if (choice == 2) {
				// 1. 데이터 입력
				// 2. 사각형 객체 생성
				// 3. 사각형의 넓이 계산 후 출력
				System.out.print(Renctangle.getName()+"너비를 입력해주세요");
				double width = sc.nextDouble();
				System.out.print(Renctangle.getName()+"높이을 입력해주세요");
				double height = sc.nextDouble();
				Renctangle rt = new Renctangle(width, height);
				System.out.print(Renctangle.getName()+"의 넓이는" + rt.calArea()+"\n");
